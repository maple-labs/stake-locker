...
while getopts d:t:r:b:v:c: flag
do
    case "${flag}" in
        d) debug=${OPTARG};;
        t) test=${OPTARG};;
...
run_type=$([ -z "$debug" ] && echo "test" || echo "debug")
...
echo LANG=C.UTF-8 dapp $run_type --match "$match" --rpc-url "$ETH_RPC_URL" --verbosity $dapp_test_verbosity --fuzz-runs $runs

LANG=C.UTF-8 dapp $run_type --match "$match" --rpc-url "$ETH_RPC_URL" --verbosity $dapp_test_verbosity --fuzz-runs $runs
